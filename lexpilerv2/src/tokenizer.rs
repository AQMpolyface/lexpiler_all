...
impl Tokenizer {
    ...
    pub fn tokenize(&mut self, content: &str) -> (u8, Vec<Token>) {
            ...
            match chars[i] {
                '=' => {
                    if i + 1 < chars.len() && chars[i + 1] == '=' {
                        self.add_token("EQUAL_EQUAL", "==", "null", self.line);
                        i += 2;
                    } else {
                        self.add_token("EQUAL", "=", "null", self.line);
                        i += 1;
                    }
                }
                '!' => {
                    if i + 1 < chars.len() && chars[i + 1] == '=' {
                        self.add_token("BANG_EQUAL", "!=", "null", self.line);
                        i += 2;
                    } else {
                        self.add_token("BANG", "!", "null", self.line);
                        i += 1;
                    }
                }
                '<' => {
                    if i + 1 < chars.len() && chars[i + 1] == '=' {
                        self.add_token("LESS_EQUAL", "<=", "null", self.line);
                        i += 2;
                    } else {
                        self.add_token("LESS", "<", "null", self.line);
                        i += 1;
                    }
                }
                '>' => {
                    if i + 1 < chars.len() && chars[i + 1] == '=' {
                        self.add_token("GREATER_EQUAL", ">=", "null", self.line);
                        i += 2;
                    } else {
                        self.add_token("GREATER", ">", "null", self.line);
                        i += 1;
                    }
                }
                '/' => {
                    if i + 1 < chars.len() && chars[i + 1] == '/' {
                        ...
                        self.add_token("SLASH", "/", "null", self.line);
                        i += 1;
                    }
                }
                '"' => {
                ...
                c if c.is_numeric() => {
                    ...
                    if numbers.contains('.') {
                        ...
                        if parts.len() == 2 && parts[1].chars().all(|c| c == '0') {
                            ...
                            self.add_token("NUMBER", &numbers, &numbers, self.line);
                        }
                    } else {
                        self.add_token("NUMBER", &numbers, &format!("{}.0", numbers), self.line);
                    }
                }
                c if c.is_alphabetic() || c == '_' => {
                    ...
                    if self.keywords.contains(&identifier) {
                        ...
                        self.add_token("IDENTIFIER", &identifier, "null", self.line);
                    }
                }
                _ => {
                    eprintln!(
        ...
        self.add_token("EOF", "", "null", self.line);
        let exit_code = if self.bad { 65 } else { 0 };
        (exit_code, self.tokens.clone())
    }

    fn add_token(&mut self, token_type: &str, lexeme: &str, literal: &str, lineuwu: u32) {
        self.tokens.push(Token {
            token_type: token_type.to_string(),
            lexeme: lexeme.to_string(),
            literal: literal.to_string(),
            line: lineuwu as usize,
        });
    }
